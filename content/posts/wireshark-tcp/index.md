...
While Wireshark calls `dissect` once per frame, as stated earlier, the `dissect` function does not have to perform actual dissection on each invocation. Instead, it also has the following options:
 ...
 1. It can set `pinfo.desegment_len` to an integer number of bytes and return — in this case Wireshark waits until the stream contains the requested number of bytes and then calls `dissect` again with a TVB containing the original segment as well as the reassembled bytes.

{{< notice note >}}
Once you have chosen the latter option (set `pinfo.desegment_len` to a number of bytes), you cannot change your decision in a later invocation of `dissect`.
{{</ notice >}}
